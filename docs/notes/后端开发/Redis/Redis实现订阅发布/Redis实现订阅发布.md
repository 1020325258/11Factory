...
Redis 的确可以使用订阅发布，但是其实并不推荐

使用 Redis 订阅发布的好处就是实现起来以及使用比较简单，适用于项目中对于订阅发布的信息的发送接收要求并不严格的情况下才可以使用，因为 Redis 中的订阅发布并不会对发布信息进行持久化，所以可能会造成数据丢失的风险，因为无法持久化，Redis 客户端必须在线才可以接收消息，且无法接收历史消息
